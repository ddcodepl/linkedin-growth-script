Linkedin = {
    config: {
        ...
        addNote: true,
        note: `Hey {{name}}, how are you?
        
I'm {{userName}}, pleasure to e-meet you!
        
I allowed myself to invite you to my business network as i think we may have common experience and can cross our business paths in the future. So let's keep that opportunity open. 
...
See you!`
        ...
        if (config.maxRequests == 0){
            ...
            if (config.addNote && config.note) {
                ...
                if(!config.debug) {
                    ...
                    console.debug("DEBUG: sipping sending invite in debug mode");
                }
            } else {
                ...
                if(!config.debug) {
                    ...
                    console.debug("DEBUG: skipping adding note in popup");
                }
            }
        }
    },
    ...
    pasteNote: function (data, config) {
        ...
        noteTextBox.value = noteTextBox.value.replace("{{userName}}", config.userName);
